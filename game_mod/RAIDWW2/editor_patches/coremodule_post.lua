-- complete copy of the code that drives the engine's `module` system, but modified so EWS can tamper with the _G table for these modules
...
function CoreModule:init()
  ...
  self.__pristine_G.EWS_Log = _G.EWS_Log
...
function CoreModule:module(module_name)
  ...
  if module_name == "CoreToolHub" or module_name == "CoreEws" then
    if self.__pristine_G.EWS == nil then
      dofile("mods/ews/ews/ews.lua")
      self.__pristine_G.EWS = _G.EWS
    end
  end
